...
def test_crud_operations():
    ...
    cursor.execute("INSERT INTO users (id, name, email) VALUES (2, 'Alice', 'alice@example.com');")
    conn.commit()
    print("Inserted row with ID 2.")
    time.sleep(3)  # Wait a few seconds to allow Debezium to capture the change

    # ---- UPDATE Operation ----
    print("Performing UPDATE operation...")
    cursor.execute("UPDATE users SET name='Alice Updated', email='alice_updated@example.com' WHERE id=2;")
    conn.commit()
